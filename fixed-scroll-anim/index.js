...
const sectionHeight = container.getBoundingClientRect().height + scrollHeight * items.length;
...
window.onscroll = () => {
  if (window.scrollY > sectionTop) {
    if (window.scrollY > sectionTop + sectionHeight - window.innerHeight) {
      ...
      section.style.top = sectionTop + sectionHeight - window.innerHeight + 'px';
    } else {
      section.style.top = '';
  ...
  const normalizedScroll = clamp((window.scrollY - sectionTop) / (sectionHeight - window.innerHeight + scrollHeight), 0, 1);
  const itemsScroll = clamp(1 + normalizedScroll * items.length, 1, items.length);
  ...
  items.forEach((item, index) => {
    if (animated.includes(index)) {
      ...
      item.style.opacity = clamp(Math.sin(itemScroll * Math.PI), 0, 0.75) * 1.33;
