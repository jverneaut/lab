...
const tiles = [
  ...
  const nextTile = tilesArr[(index + tilesArr.length + 1) % tilesArr.length] || {};
...
const animate = () => {
  ...
  const currentTile = tiles[(currentTileIndex + tiles.length) % tiles.length] || {};
  const nextTile = tiles[(currentTileIndex + tiles.length + 1) % tiles.length] || {};
  ...
  switch (dir) {
    case 'down':
      container.style.transform = `translate3d(${-scrollX}px, ${-(scrollY + tileScroll)}px, 0)`;
      break;
    case 'up':
      container.style.transform = `translate3d(${-scrollX}px, ${-(scrollY - tileScroll)}px, 0)`;
      break;
