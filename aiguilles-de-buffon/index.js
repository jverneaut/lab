...
document.querySelector('.graph span:nth-of-type(1)').innerHTML = graphMax * 100 + '%';
document.querySelector('.graph span:nth-of-type(2)').innerHTML =
  0.5 * (graphMin + graphMax) * 100 + '%';
document.querySelector('.graph span:nth-of-type(3)').innerHTML = graphMin * 100 + '%';
...
const draw = () => {
  ...
  matches.slice(matches.length - newMatchesNumber, matches.length).forEach(match => {
    match.draw(ctx);
  });
  ...
  const estimate = (2 * ALL_LENGTH * totalMatches) / (intersectingMatches * floorWidth);

  const piStr = estimate.toString().slice(0, 1) + estimate.toString().slice(2, 11);
  ...
  document.querySelector('h3').innerHTML =
    totalMatches.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ') + ' aiguilles';
  ...
  graphPoints.forEach((graphPoint, index) => {
    ...
    const point = Math.max(0, (HEIGHT / 2) * (1 - (graphPoint - graphMin) / graphMax));
    const prevPoint = graphPoints[index - 1]
      ? Math.max(0, (HEIGHT / 2) * (1 - (graphPoints[index - 1] - graphMin) / graphMax))
      : 0;
