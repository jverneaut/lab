...
const textureCoords = textureIndices.map(index => texturePositions[index]).flat();
...
const draw = () => {
  images.forEach((image, index) => {
    ...
    // Now, we do the same as the texture, i.e. dividing our mesh
    // into multiple triangle strip rows.
    const vertexPositions = [];
    for (let i = 0; i < MESH_HEIGHT_SEGMENTS_NUMBER; i++) {
      for (let j = 0; j < MESH_WIDTH_SEGMENTS_NUMBER; j++) {
        vertexPositions.push([
          corners.left +
            (j * (corners.right - corners.left)) / (MESH_WIDTH_SEGMENTS_NUMBER - 1),
          corners.top +
            (i * (corners.bottom - corners.top)) / (MESH_HEIGHT_SEGMENTS_NUMBER - 1),
        ]);
      }
    }
